...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Evolving-Machine thinks for you about the meanings.",
        epilog="Example:  evolving-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | evolving-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | evolving-machine         #

    $ evolving-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="evolving-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"evolving-machine: {utterance}.")
        syslog.closelog()
